class SMLib {
	...
	get firstWeekTeamMembers() {
		let memberIndex = 0;
		const members = [];
		const fixedDayMembers = this.activeTeamMembers.filter(m => Number.isInteger(m.fixedDay) && m.fixedDay >= 1 && m.fixedDay <= 5);
		const tempTeamMembers = [...this.activeTeamMembers];
		if (tempTeamMembers.length > 0) {
			while (members.length < 5) {
				let member = fixedDayMembers.find(m => m.fixedDay === members.length + 1);
				if (member) {
	...
	getAllHolidayName(date) {
		...
		const uniqueReasons = [...new Set(rulesForDate.map(r => r.reason).filter(Boolean))];
